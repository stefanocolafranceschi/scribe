...
$elogmessagefile='/srsconfig/elogmessage';
   ...
   if (file_exists($elogmessagefile)) {
      $sendanelog = "/usr/local/bin/elog -v -h ".$eloghost." -p ".$elogport." -l fit -a Author=".$eloguser." -a Type=''Configuration'' -a Subject=''Configuration'' -m ".$elogmessagefile." -f /srsconfig/config.zip > /dev/null";
      //echo ciao1;
      system($sendanelog);
      system("rm ".$elogmessagefile);
   }
   else {
      $sendanelog = "/usr/local/bin/elog -v -h ".$eloghost." -p ".$elogport." -l fit -a Author=".$eloguser." -a Type=''Configuration'' -a Subject=''Configuration saved'' Automatic saved -f /srsconfig/config.zip > /dev/null";
      //echo $sendanelog;
      system($sendanelog);
   }
